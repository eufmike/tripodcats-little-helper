...
def segment2array(ipfilelist, maxlength, nonsilence_dict, buffer=10):
    ...
    for i, ipfile in enumerate(ipfilelist):
        ...
        for start, end in sections_times:
            ...
            if start_with_buffer < 0:
                start_with_buffer = 0
            if end_with_buffer > maxlength:
                end_with_buffer = maxlength
...
def detect_no_silence(
    ipaudio_dict, min_silence_len, silence_thresh, threads, logger
):
    # multiple processing
    if threads > 1:
        ...
        for ipfile, value in ipaudio_dict.items():
            pool.apply_async(
                _detect_no_silence,
                args=(ipfile, value, min_silence_len, silence_thresh, logger),
        ...
        for ipfile, value in tqdm(ipaudio_dict.items()):
            ipfile, sections_times = _detect_no_silence(
                ipfile, value, min_silence_len, silence_thresh, logger
            )
            nonsilence_dict[ipfile] = sections_times
...
def remove_silence(
    ipaudio_dict, list_nosilence, random_silence, bitrate, threads, logger
):
    # multiple processing
    if threads > 1:
        pbar = tqdm(total=len(ipaudio_dict.keys()))
...
def silrm(args):
    ...
    if not tmp_pkl.is_file():
        print("tmp_pkl is not file")
        nonsilence_dict = detect_no_silence(
            ipaudio_dict, min_silence_len, silence_thresh, threads, logger
        )
        ...
        logger.info(
            f"Load non-silence segments from previous processed pkl file: {tmp_pkl}"
        )

    array_nosilence = segment2array(
        ipfilelist, maxlength, nonsilence_dict, buffer=10
    )
...
def add_subparser(subparsers):
    ...
    optional_group.add_argument(
        ...
        "--output_dir",
        type=str,
        help="output folders different from default",
    )
    optional_group.add_argument(
        "-pz",
        "--pad_zero",
        action="store_true",
        help="pad zero to the end of audio",
    )
    optional_group.add_argument(
